...
```{r nhanessimlrstat,eval=TRUE,fig.width=10,fig.height=8}
# drwwrd
projlist=list()
mysimk=ncol(initu)
...
#  if (doimp) mdl = lm( myform, data=tempdf, weights=tempdf$wtint2yr ) else mdl = lm( myform, data=tempdf, weights=tempdf$wtint2yr  )
  ...
  if ( myanv$Pr[2] < basep/mysimk ) {
    ...
    for ( k in names(projlist) ) {
      ...
      gglist[[length(gglist)+1]]=visreg::visreg(mdl, paste0(k,kk), gg=TRUE ) + ggtitle(ttl) + theme_minimal()
      }
    print( grid.arrange( grobs=gglist ) )
    cat("*** result end ***********************************\n\n")
#    Sys.sleep(3)
  }
}
