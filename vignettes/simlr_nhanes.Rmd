...
colcats[ colnames(fdata) %in%  getNamesFromDataframe( "LBX", fdata, exclusions='LC' ) ]='exposures'
colcats[ colnames(fdata) %in%  getNamesFromDataframe( "LBD", fdata, exclusions='LC' ) ]='exposures'
colcats[ colnames(fdata) %in%  getNamesFromDataframe( "URX", fdata, exclusions='LC' ) ]='exposures'
colcats[ colnames(fdata) %in%  getNamesFromDataframe( "URD", fdata, exclusions='LC' ) ]='exposures'
colcats[ colnames(fdata) %in%  getNamesFromDataframe( "LBC", fdata, exclusions='LC' ) ]='exposures'
...
dnames = c( "riagendr", "ridageyr", "race", "dmdeduc2", "INDFMPIR")
if ( doimp ) {
...
for ( kk in 1:length(cognames)) {
  myform = paste0( cognames[kk]," ~ riagendr + ridageyr + race + dmdeduc2 + INDFMPIR + ",paste0( colnames(simdf), collapse='+'))
  tempdf = simdf2[mycc,]
  if (doimp) mdl = lm( myform, data=tempdf ) else mdl = lm( myform, data=tempdf )
  print( summary( mdl ) )
  print( cognames[kk])
  Sys.sleep(3)
}
...
for ( k in names(projlist) ) {
  ...
  if ( mysimk > 1 ) print( interpret_simlr_vector2( resultNH$v[[k]], 2, n2show=3, shortnames=F ) )
}
##########################################################################################
